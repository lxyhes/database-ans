...
electron.ipcMain.handle("api-call", async (event, { method, endpoint, data }) => {
  var _a, _b;
  try {
    const url2 = `${BACKEND_URL}${endpoint}`;
    let response;
    switch (method.toUpperCase()) {
      case "GET":
        ...
        break;
      case "POST":
        response = await axios.post(url2, data);
        break;
      case "PUT":
        response = await axios.put(url2, data);
        break;
      case "DELETE":
        response = await axios.delete(url2);
        break;
      default:
...
electron.ipcMain.handle("query-natural-language", async (event, query) => {
  try {
...
electron.ipcMain.handle("analyze-data", async (event, query) => {
  try {
...
electron.ipcMain.handle("get-data-summary", async () => {
  try {
...
electron.ipcMain.handle("get-table-schema", async (event, tableName) => {
  try {
...
electron.ipcMain.handle("get-all-tables", async () => {
  try {
...
electron.ipcMain.handle("get-query-history", async () => {
  try {
...
electron.ipcMain.handle("save-query", async (event, queryData) => {
  try {
...
electron.ipcMain.handle("toggle-favorite", async (event, id, isFavorite) => {
  try {
...
electron.ipcMain.handle("iflow-query", async (event, prompt, functionType = "query") => {
  try {
    const endpointMap = {
...
electron.ipcMain.handle("code-assistant", async (event, action, data) => {
  try {
    const endpointMap = {
...
electron.ipcMain.handle("import-file", async (event, formData) => {
  try {
...
electron.ipcMain.handle("export-data", async (event, sql, format) => {
  try {
